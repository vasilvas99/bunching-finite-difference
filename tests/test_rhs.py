...
# (function type, k_a value)
VARIANTS = [
    (RHSType.MM2_a0p1_r1_jax, 0.1),
    (RHSType.MM2_a1_r1_jax, 1.0),
    (RHSType.MM2_a10_r1_jax, 10.0),
    (RHSType.MM2_a50_r1_jax, 50.0),
    (RHSType.MM2_a100_r1_jax, 100.0),
    (RHSType.MM2_a200_r1_jax, 200.0),
]


@pytest.mark.parametrize("f_type, ka", VARIANTS, ids=lambda v: str(v))
def test_equally_spaced_steps_give_zero_velocity(f_type, ka):
...
@pytest.mark.parametrize("f_type, ka", VARIANTS, ids=lambda v: str(v))
def test_antisymmetry_under_terrace_swap(f_type, ka):
...
@pytest.mark.parametrize("f_type, ka", VARIANTS, ids=lambda v: str(v))
def test_matches_analytical_mm2_formula(f_type, ka):
...
RANDOM_ABC: list[tuple[float, float, float]] = []
for _ in range(100):
    _a = random.uniform(0.0, 10.0)
    _b = _a + random.uniform(0.1, 10.0)   # Δu_n  ∈ [0.1, 10]
    _c = _b + random.uniform(0.1, 10.0)   # Δu_{n+1} ∈ [0.1, 10]
    RANDOM_ABC.append((_a, _b, _c))
...
@pytest.mark.parametrize(
    "a, b, c",
    RANDOM_ABC,
    ids=[f"a={a:.2f}_b={b:.2f}_c={c:.2f}" for a, b, c in RANDOM_ABC],
)
def test_matches_analytical_mm2_formula_random(f_type, ka, a, b, c):
    delta_n = b - a
    delta_np1 = c - b
    f = f_type.into_rhs()
    expected = (
